...
@Controller
@RequestMapping("/sys")
public class RegisterSysController {
    ...
    public void ajaxLoginName(String loginName, HttpServletResponse response) throws IOException {
        //校验登录名是否已经存在
        //根据用户名查询用户
        UserEntity user = userSysService.findByLoginNameAndPass(loginName, null);
    ...
    public String submitTable(UserEntity userEntity) {
        ...
        userSysService.insert(user);
        return "redirect:/sys/register/index";
    }
}
